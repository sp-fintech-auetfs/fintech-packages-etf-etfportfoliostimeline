...
namespace Apps\Fintech\Packages\Etf\Portfoliostimeline;
...
class EtfPortfoliostimeline extends BasePackage
{
    public $snapshotsModel = AppsFintechEtfPortfoliostimelineSnapshots::class;

    public $performancesChunksModel = AppsFintechEtfPortfoliosPerformancesChunks::class;

    public $portfolioSchemes = [];

    public $parsedCarbon = [];

    public $etfportfoliostimeline;

    public $timelineDateBeingProcessed;

    public $portfolio;

    protected $modelToUse = AppsFintechEtfPortfoliostimeline::class;

    protected $packageName = 'etfportfoliostimeline';
    ...
    public function getPortfoliotimelineByPortfolio($portfolio, $getCached = true)
    {
        ...
        if ($this->opCache && $getCached) {
            ...
            // if (!$this->opCache->checkCache($this->timeline['id'], 'etfportfoliostimeline')) {
            //     $this->opCache->setCache($this->timeline['id'], $this->timeline, 'etfportfoliostimeline');
            // }

            // if ($cachedTimeline = $this->opCache->getCache($this->timeline['id'], 'etfportfoliostimeline')) {
    ...
    public function getPortfoliotimelineByPortfolioAndTimeline($portfolio, $getTimelineDate = null, $force = false)
    {
        ...
        if (isset($this->timeline['snapshots_ids'][$getTimelineDate]) && !$force) {
            try {
                if ($this->localContent->fileExists('.ff/sp/apps_fintech_etf_portfoliostimeline_snapshots/data/' . $this->timeline['snapshots_ids'][$getTimelineDate] . '.json')) {
                    $timelineSnapshot = $this->helper->decode($this->localContent->read('.ff/sp/apps_fintech_etf_portfoliostimeline_snapshots/data/' . $this->timeline['snapshots_ids'][$getTimelineDate] . '.json'), true);
                }
        ...
        if (!isset($timelinePortfolio)) {
            ...
            try {
                $this->localContent->write(
                    '.ff/sp/apps_fintech_etf_portfoliostimeline_snapshots/data/' . $timelineSnapshot['snapshot']['id'] . '.json',
        ...
        if ($timelinePortfolio) {
            ...
            try {
                $this->localContent->write(
                    '.ff/sp/apps_fintech_etf_portfoliostimeline/data/' . $this->timeline['id'] . '.json',
    ...
    {
        if ($this->basepackages->progress->checkProgressFile('etfportfoliotimeline')) {
            $this->basepackages->progress->deleteProgressFile('etfportfoliotimeline');
        }
    ...
    public function processTimelineNeedsGeneration($data)
    {
        ...
        foreach ($portfolioTransactions as $portfolioTransaction) {
            if (!isset($this->portfolioSchemes[$portfolioTransaction['scheme_id']])) {
                $scheme = $this->schemePackage->getSchemeFromEtfCodeOrSchemeId($portfolioTransaction, true);
        ...
        $progressFile = $this->basepackages->progress->checkProgressFile('etfportfoliotimeline');
    ...
    protected function generatePortfolioTimeline($args)
    {
        ...
        if (!isset($this->timeline['snapshots_ids'][$dateToProcess]) || $forceRecalculateTimeline) {
            ...
            if (isset($this->timeline['snapshots_ids'][$dateToProcess])) {
                try {
                    if ($this->localContent->fileExists('.ff/sp/apps_fintech_etf_portfoliostimeline_snapshots/data/' . $this->timeline['snapshots_ids'][$dateToProcess] . '.json')) {
                        $timelineSnapshot['snapshot']['id'] = $this->timeline['snapshots_ids'][$dateToProcess];
                        $timelineSnapshot['id'] = $timelineSnapshot['snapshot']['id'];

                        $this->localContent->write(
                            '.ff/sp/apps_fintech_etf_portfoliostimeline_snapshots/data/' . $this->timeline['snapshots_ids'][$dateToProcess] . '.json',
                            $this->helper->encode($timelineSnapshot)
                        );
                    } else {
                        $timelineSnapshot['snapshot']['id'] = $this->getLastInsertedId() + 1;
    ...
    protected function generatePortfolioPerformance($args)
    {
            ...
            foreach (['week', 'month', 'threeMonth', 'sixMonth', 'year', 'threeYear', 'fiveYear', 'tenYear', 'fifteenYear', 'twentyYear', 'twentyFiveYear', 'thirtyYear', 'all'] as $time) {
                ...
                if (count($snapshotChunks) > 0) {
                    ...
                    foreach ($snapshotChunks as $snapshotChunkDate => $snapshotId) {
                        try {
                            if ($this->localContent->fileExists('.ff/sp/apps_fintech_etf_portfoliostimeline_snapshots/data/' . $snapshotId . '.json')) {
                                $timelineSnapshot =
                                    $this->helper->decode(
                                        $this->localContent->read(
                                            '.ff/sp/apps_fintech_etf_portfoliostimeline_snapshots/data/' . $snapshotId . '.json'
                                        ), true
                                    )['snapshot'];
                            }
        ...
        if (isset($this->portfolio['performances_chunks'])) {
            try {
                if ($this->localContent->fileExists('.ff/sp/apps_fintech_etf_portfolios_performances_chunks/data/' . $this->portfolio['performances_chunks']['id'] . '.json')) {
                    $chunks['id'] = $this->portfolio['performances_chunks']['id'];

                    $this->localContent->write(
                        '.ff/sp/apps_fintech_etf_portfolios_performances_chunks/data/' . $this->portfolio['performances_chunks']['id'] . '.json',
                    ...
                    try {
                        $this->localContent->write(
                            '.ff/sp/apps_fintech_etf_portfolios_performances_chunks/data/' . $chunks['id'] . '.json',
                            $this->helper->encode($chunks)
                        );
    ...
    {
        if (!$model) {
            $this->setModelToUse($this->modelToUse = AppsFintechEtfPortfoliostimeline::class);

            $this->packageName = 'etfportfoliostimeline';
        } else {
